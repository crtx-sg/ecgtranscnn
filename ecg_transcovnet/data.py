...
from .preprocessing import FilterConfig, PreprocessingPipeline, FILTER_PRESETS
from .classes import canonical_condition_name
...
def generate_dataset(
    ...
    filter_config : FilterConfig or None
        Preprocessing filter configuration.  ``None`` applies z-score
        normalization only (backward compatible).
    ...
    if filter_config is None:
        filter_config = FILTER_PRESETS["none"]
    pipeline = PreprocessingPipeline(filter_config)
...
def load_or_generate_data(
    ...
    # Include filter preset in cache key to avoid stale cached data
    filter_tag = "none"
    if filter_config is not None:
        for name, preset in FILTER_PRESETS.items():
            if preset == filter_config:
                filter_tag = name
                break
        else:
            # Custom config — use a hash of the repr
            filter_tag = f"custom{hash(repr(filter_config)) % 100000:05d}"
    cache_path = (
        Path(cache_dir)
        / f"ecg_{leads_tag}_{noise_level}_{distribution}_f{filter_tag}_t{num_train}_v{num_val}_s{seed}.npz"
    )
...
def load_hdf5_test_samples(
    ...
    The label comes from the filename prefix; files without a known prefix use
    each event's ``condition`` attribute (enum value or name).
    ...
    for h5_file in sorted(test_path.glob("*.h5")):
        ...
        for prefix, cond in LABEL_TO_CONDITION.items():
            if fname.startswith(prefix):
                condition = cond
                break
        file_class_idx = CONDITION_TO_IDX[condition] if condition is not None else None
        ...
        with h5py.File(h5_file, "r") as f:
            ...
            for event_key in sorted(f.keys()):
                ...
                if "ecg" not in event:
                    continue
                class_idx = file_class_idx
                if class_idx is None:
                    name = canonical_condition_name(event.attrs.get("condition"))
                    if name is None:
                        continue
                    class_idx = CONDITION_TO_IDX[Condition[name]]
                ...
                samples.append((fname, signal, class_idx))
...
def evaluate_hdf5_test(
    ...
    *class_names* is the model head (default: 16 simulator classes); samples
    whose label is not in the head are skipped.
    """
    names = list(class_names) if class_names is not None else CLASS_NAMES
    model.eval()
    samples = load_hdf5_test_samples(test_dir, leads, filter_config=filter_config)
    if not samples:
    ...
    correct = 0
    evaluated = 0
    for fname, signal, enum_idx in samples:
        if CLASS_NAMES[enum_idx] not in names:
            continue
        true_idx = names.index(CLASS_NAMES[enum_idx])
        evaluated += 1
    ...
    print(f"\n  Test accuracy: {correct}/{evaluated} = {acc:.1%}")
